...
public class frameListener   implements GoBangconfig,MouseListener,UserStrategy{
	int tx=0;
	int ty=0;
	...
	public Integer unionWeight(Integer a,Integer b ) {
	  ...
	  public int judgeBlack(int AIi, int AIj) {
		...
		// 直接搜索算法，
		//？盲目搜索法的改进（利用权值表map）避免使用栈O(4^n)4的15次方
		//权值法，无启发式函数，
		//无剪枝
		...
		//不用map权值表
		//没有明确的目标状态
		//如何定权：搜索该棋子周围5x5个方格，判断周围有多少个与其同色的棋子，将该数值作为该棋子的权值
